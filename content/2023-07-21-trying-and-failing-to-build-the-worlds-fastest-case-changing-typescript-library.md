...
After a little poking around, I decided I'd try to iterate through the raw byte array of the input string and build a new string in-place rather than splitting on word boundaries and then looping through repeatedly. My original algorithm had linear time complexity, but it was performing at least 2 loops: once to split on word boundaries, then again on each word to adjust the casing. I was hoping to reduce the number of loops needed to generate the transformed string. Space complexity would probably be linear in both cases because we're building a new string rather than modifying the string in-place. Modifying the string in-place didn't seem feasible because of the need to insert delimiters, for example when converting `PascalCase` to `snake_case`. If you know of a way to modify strings in-place in Rust including inserting characters, please let me know!
